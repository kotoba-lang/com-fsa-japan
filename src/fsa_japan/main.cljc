(ns fsa_japan.main
  "Kotodama WASM entrypoint for the Fsa_japan clean-room actor (L4) — Clojure port.
  ...
  facts (`fsa_japan.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fsa_japan")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"fsa_japan.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fsa_japan-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fsa_japan-compat/ready)
